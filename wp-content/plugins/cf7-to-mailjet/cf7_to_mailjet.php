			...
			&& !empty($options_list))
	{
		...
		foreach ($lists_to_subscribe as $sub)
		{
			...
			if (!class_exists('WP_Mailjet_Api_Strategy_V3')) {
				...
				{
					if (is_array($data[$custom_label_2]))
						$data_to_mailjet = implode(' ', $data[$custom_label_2]);
					else
				...
				{
					if (is_array($data[$custom_label_3]))
						$data_to_mailjet = implode(' ', $data[$custom_label_3]);
					else
						$data_to_mailjet = $data[$custom_label_3];
				...
				{
					if (is_array($data[$custom_label_4]))
						$data_to_mailjet = implode(' ', $data[$custom_label_4]);
					else
						$data_to_mailjet = $data[$custom_label_4];
				...
				{
					if (is_array($data[$custom_label_5]))
						$data_to_mailjet = implode(' ', $data[$custom_label_5]);
					else
						$data_to_mailjet = $data[$custom_label_5];
			...
			{
				if (isset($data[$checkbox]) && $email != "" && $list != "")
				{
					mailjet_add_to_list($list, $contacts);
				}
			}
